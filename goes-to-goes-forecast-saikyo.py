...
class Predictor(chainer.Chain):
    def __init__(self):
        super(Predictor, self).__init__(
            ...
            l3=L.Linear(hidden_layer_size,hidden_layer_size),
            l9=L.Linear(hidden_layer_size,1)
        )
...
contingency_table = {}
flare_classes = ["C","M","X"]
def flare_threshold(c):
    if c=="C": return 1e-6
    if c=="M": return 1e-5
    if c=="X": return 1e-4
    raise Exception("Unknown flare class")

for p in [False,True]:
    for o in [False,True]:
        for c in flare_classes:
            contingency_table[(c,p,o)] = 0
...
def predict(training_mode = True):
    ...
    if training_mode:
        ...
        # また、contingency_tableを作ります。
        for c in flare_classes:
            threshold_v = flare_threshold(c)
            contingency_table[(c,predict_v.data[0,0]>threshold_v,observe_v.data[0,0]>threshold_v)]+=1
