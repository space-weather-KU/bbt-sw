...
dvi_file=$filename_body.dvi ## Random temp file name
pdf_file=$filename_body.pdf ## Random temp file name
...
echo load-image.py get-sdo-image.py convolution.py convolution-2.py cat-or-dog.py | xargs -n 1 echo |
...
#platex -output-directory /tmp $tex_file &&
#platex -output-directory /tmp $tex_file && ## This needs to be run twice
## for the TOC to be generated
#dvipdfmx  -o $pdf_file $dvi_file
lualatex -output-directory /tmp $tex_file &&
lualatex -output-directory /tmp $tex_file &&
mv $filename_body.pdf all-sources.pdf
pdftk sample-programs.pdf all-sources.pdf cat output sample-programs-all.pdf
